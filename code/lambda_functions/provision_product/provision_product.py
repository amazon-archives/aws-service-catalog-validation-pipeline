...
def lambda_handler(event, context):
    try:
        logger.config(context.aws_request_id, loglevel='debug', botolevel='debug')
        ...
        if not stacks_in_event:
            logger.info({'event': 'provision_product'})
            stacks = pipeline_run.provision_product()
        else:
            logger.info({'event': 'provision_product_continuation'})
        ...
        if len(stacks['inprogress']) > 0:
            msg = "Stack creation still in progress: %s" % stacks['inprogress']
            data = {
                "status": msg,
            ...
            msg = "%s ProvisionProduct failures %s" % (len(stacks['error']), stacks['error'])
            pipeline_run.put_job_failure(msg)
            logger.debug("%s ProvisionProduct failures %s" % (len(stacks['error']), stacks['error']))
            return
        logger.info({'event': 'stack_create_complete', "stacks": stacks['success']})
        pipeline_run.upload_output_artifact(stacks['success'])
        msg = "%s ProvisionProduct success %s" % (len(stacks['success']), stacks['success'])
